...
Note that most of the documentation of the code is written in the
interface files (=.mli=).
* Lexer
** Lexical Units
...
code, which is described in section \ref{sec:Codegen}. The advantage
of using an abstract syntax tree is that both the decoration of the
grammar and the code generation are simplified. The decoration of the
grammar already does some simplifications, which avoid having to take
lots of different cases into account during code generation (eg. a
=unless x= condition is translated in the same way as would be an =if
(not x)= condition).
...
each non-terminal is represented by a function that parses it from a
stream.

The type signature of the parser is the following:
...
stream emitted by the parser, with the errors removed), and returns
three things: a list of function definitions, a list of instructions,
and a symbol table. The symbol table contains the list of global
variables and the arity of the functions, and will be used by the code
generator.
...
** Values
The different values are represented as follow:
  - A string is represented by its pointer, which is a multiple of 4,
    since we store each string word-aligned
...
being prepended by the declaration of all those strings. This allow to
...
The assignments simply consists of storing a new value into the
address of the destination variable.
** Binary operations
A binary operation =BinOp(op, e1, e2)= is computed as follow:
...
There are three possible unary operations:
  - *Unary plus*: it in fact doesn't do anything, so the code
    generated is the same as the code without this unary operation
...
executed if the condition is truu), followed by a jump to the end
...
A function definition is generated as follows:
  ...
  - the name of the function arguments are also stored in the code
...
first have a working program without needing a code generator, and
thus refine the grammar and abstract syntax tree early in the
...
then the chances that the generated program is correct are high.
